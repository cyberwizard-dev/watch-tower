...
import { ArrowDown, ArrowUp, ArrowUpDown, ExternalLink, Mail, Search } from 'lucide-react';
...
type Props = {
    summary: Summary;
    mails: Paginated<MailRow>;
    selectedRange: string;
    filters: { search: string | null; sort: SortKey; dir: SortDir };
};

type SortKey = 'mailable_class' | 'count' | 'avg_ms' | 'p95_ms';
type SortDir = 'asc' | 'desc';

export default function MailIndex({ summary, mails, selectedRange, filters }: Props) {
    ...
    const toggleSort = (key: SortKey) => {
        const nextDir: SortDir = sortKey === key ? (sortDir === 'asc' ? 'desc' : 'asc') : key === 'mailable_class' ? 'asc' : 'desc';
    ...
    return (
        <AppLayout title="Mail">
            <PageHeader title="Mail" breadcrumbs={[{ label: 'Activity' }, { label: 'Mail' }]} selectedRange={selectedRange} />

            <div className="space-y-6 px-6 py-6">
                <div className="grid gap-6 lg:grid-cols-2">
                    <MailsCard summary={summary} />
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                        <CardTitle className="flex items-center gap-2">
                            ...
                            <span>{mails.total.toLocaleString()} Mails</span>
                        </CardTitle>
                    ...
                    <CardContent className="p-0">
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <SortableHead label="MAIL" sortKey="mailable_class" current={sortKey} dir={sortDir} onSort={toggleSort} />
                                ...
                                {mails.data.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={5} className="py-10 text-center text-sm text-muted-foreground">
                                            No mail captured in {selectedRange.toUpperCase()}
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    mails.data.map((row) => <MailRowTr key={row.hash} row={row} slug={slug} />)
                                )}
                            </TableBody>
                        </Table>
                        <Pagination links={mails.links} from={mails.from} to={mails.to} total={mails.total} />
                    </CardContent>
                </Card>
            </div>
        </AppLayout>
...
function MailRowTr({ row, slug }: { row: MailRow; slug: string }) {
    const href = mailRoutes.show([slug, row.hash]).url;
        ...
        <TableRow className="cursor-pointer hover:bg-muted/50">
            <TableCell className="py-2.5">
                <Link href={href} className="flex items-center gap-2 font-mono text-xs hover:text-foreground">
                    ...
                    <span className="block max-w-[640px] truncate" title={row.mailable_class}>
                        {row.mailable_class}
                    </span>
                </Link>
            </TableCell>
...
function MailsCard({ summary }: { summary: Summary }) {
            ...
            <CardContent className="p-5">
                <div className="flex items-baseline gap-6">
                    <div>
                        ...
                        <div className="text-[10px] tracking-wider text-muted-foreground uppercase">Mails</div>
                    </div>
                </div>
