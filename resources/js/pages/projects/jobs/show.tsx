...
export default function JobsShow({ detail, selectedRange }: Props) {
    ...
    const filteredAttempts = useMemo(() => {
        return detail.attempts.filter((a) => {
            if (statusFilter === 'processed' && a.status !== 'completed') return false;
            if (statusFilter === 'released' && a.status !== 'released') return false;
            if (statusFilter === 'failed' && a.status !== 'failed') return false;

            if (durationFilter === 'avg' && detail.duration.avg_ms !== null && a.duration_ms !== null) {
                if (a.duration_ms < detail.duration.avg_ms) return false;
            }
            if (durationFilter === 'p95' && detail.duration.p95_ms !== null && a.duration_ms !== null) {
                if (a.duration_ms < detail.duration.p95_ms) return false;
            }

            return true;
        });
    }, [detail.attempts, detail.duration.avg_ms, detail.duration.p95_ms, statusFilter, durationFilter]);
                ...
                title={
                    <div className="space-y-0.5">
                        <div className="text-[11px] font-normal uppercase tracking-wider text-muted-foreground">
                            <Link href={jobsRoutes.index(slug).url} className="hover:text-foreground">
                                Jobs
                            </Link>
                        </div>
            ...
            <div className="space-y-6 px-6 py-6">
                ...
                <Card>
                    <CardHeader className="flex flex-row flex-wrap items-center justify-between gap-3 space-y-0 pb-3">
                        <CardTitle>{filteredAttempts.length} Attempts</CardTitle>
                        <div className="flex items-center gap-3">
                            ...
                            <Tabs value={statusFilter} onValueChange={(v) => setStatusFilter(v as StatusFilter)}>
                                <TabsList className="h-8">
                                    <TabsTrigger value="all" className="px-3">View all</TabsTrigger>
                                    <TabsTrigger value="processed" className="px-3 gap-1.5">
                                        Processed
                                        <Badge variant="muted" className="px-1.5 text-[10px]">{detail.totals.processed}</Badge>
                                    </TabsTrigger>
                                    <TabsTrigger value="released" className="px-3 gap-1.5">
                                        Released
                                        {detail.totals.released > 0 ? (
                                            <Badge variant="muted" className="px-1.5 text-[10px]">{detail.totals.released}</Badge>
                                        ) : null}
                                    </TabsTrigger>
                                ...
                                {filteredAttempts.length === 0 ? (
                                    ...
                                    filteredAttempts.map((attempt) => (
                                        <TableRow key={attempt.id}>
                                            ...
                                            <TableCell className="py-2.5 font-mono text-xs">{attempt.attempt}</TableCell>
        ...
        <Card>
            ...
            <CardContent className="p-5">
                <div className="flex items-baseline gap-6">
                    ...
                    <Stat label="Released" value={formatNumber(totals.released)} accent="text-amber-600 dark:text-amber-400" dot="bg-amber-500" />
                </div>
...
function JobStatusBadge({ status }: { status: string }) {
    const map: Record<string, { label: string; cls: string }> = {
        completed: { label: 'PROCESSED', cls: 'border-emerald-500/40 text-emerald-600 dark:text-emerald-400' },
        released: { label: 'RELEASED', cls: 'border-amber-500/40 text-amber-600 dark:text-amber-400' },
        failed: { label: 'FAILED', cls: 'border-rose-500/40 text-rose-600 dark:text-rose-400' },
        queued: { label: 'QUEUED', cls: 'border-sky-500/40 text-sky-600 dark:text-sky-400' },
    };
